...
function handlePostSecret(req, res) {
   try {
      data.secrets[req.peerCN] = req.body;
      var reply = {
         message: 'ok',
         user: req.peerCN,
         custodianCount: data.custodianCount,
...
function appVerifier(req, res, next) {
   log.info('appVerifier', req.url, res.socket.authorized);
   if (!res.socket.authorized) {
      ...
      log.info('appVerifier', req.url, cert.subject.CN, cert.issuer);
      next();
   }
}
