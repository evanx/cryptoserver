...
GenerateKey.prototype = {
   ...
   encryptDuo: function (duo, clearSecret, callback) {
      ...
      cryptoFunctions.pbkdf2(clearSecret, this.salt, function (err, kek) {
         if (err) {
            instance.log.error('pbkdf2 error', duo, err);
            callback(err);
         } else {
            instance.log.error('generatedDek', instance.generatedDek.length);
            var cipher = cryptoFunctions.createCipheriv(kek, instance.iv);
            var encryptedDek = cryptoFunctions.encrypt(cipher, instance.generatedDek);
            var decipher = cryptoFunctions.createDecipheriv(kek, instance.iv);
