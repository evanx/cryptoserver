...
The field `dek:brent:evan` et al is the encrypted DEK. It is encrypted using a KEK that is derived using the <i>split knowledge</i> of two custodians. As such two custodians are required to decrypt the key, i.e. <i>dual control.</i> Clearly the DEK is "known to no single person" (in clear-text) as per the PCI DSS requirements. 
